...
from commisery.commit import BREAKING_CHANGE_TOKEN, CommitMessage, ConventionalCommit
...
def C002_one_whiteline_between_subject_and_body(message: COMMIT, _: Configuration):
...
def C003_title_case_description(message: COMMIT, config: Configuration):
...
def C004_unknown_tag_type(message: COMMIT, config: Configuration):
...
def C005_separator_contains_trailing_whitespaces(message: COMMIT, config: Configuration):
...
def C006_scope_should_not_be_empty(message: COMMIT, _: Configuration):
...
def C007_scope_contains_whitespace(message: COMMIT, config: Configuration):
...
def C008_missing_separator(message: COMMIT, _: Configuration):
...
def C009_missing_description(message: COMMIT, _: Configuration):
...
def C010_breaking_indicator_contains_whitespacing(message: COMMIT, _: Configuration):
...
def C011_only_single_breaking_indicator(message: COMMIT, _: Configuration):
...
def C012_missing_type_tag(message: COMMIT, _: Configuration):
...
def C013_subject_should_not_end_with_punctuation(message: COMMIT, config: Configuration):
...
def C014_subject_exceeds_line_lenght_limit(message: COMMIT, config: Configuration):
...
def C015_no_repeated_tags(message: COMMIT, config: Configuration):
...
def C016_description_in_imperative_mood(message: COMMIT, config: Configuration):
...
def C017_subject_contains_review_remarks(_: COMMIT, __: Configuration):
    """Subject should not contain reference to review comments"""
    # TODO: implement this rule


def C018_missing_empty_line_between_subject_and_body(message: COMMIT, _: Configuration):
...
def C022_footer_contains_blank_line(message: COMMIT, _: Configuration):
...
def C023_breaking_change_must_be_first_git_trailer(message: COMMIT, _: Configuration):
...
def validate_commit_message_rule(rule: str, message: COMMIT, config: Configuration):
